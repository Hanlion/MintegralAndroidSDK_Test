...
public class BidInterstitialVideoActivity extends BaseActivity implements OnClickListener {
	...
	private MTGBidInterstitialVideoHandler mMtgInterstitalVideoHandler;
	...
	public void setListener() {
		mTitleLayout.setTitleText("Head Bid InterstitialVideo");
	...
	private void initHandler() {
		try {
			...
			//mMtgInterstitalVideoHandler = new MTGBidInterstitialVideoHandler(this, mRewardUnitId);
			mMtgInterstitalVideoHandler = new MTGBidInterstitialVideoHandler(this, "138781", mRewardUnitId);
			mMtgInterstitalVideoHandler.setInterstitialVideoListener(new InterstitialVideoListener() {
				...
				public void onAdCloseWithIVReward(boolean isComplete, int rewardAlertStatus) {
					...
					Toast.makeText(getApplicationContext(),"onAdCloseWithIVReward",Toast.LENGTH_LONG).show();
			...
			manager = new BidManager("138781", mRewardUnitId);
		...
		switch (v.getId()) {
			...
			case R.id.bt_load:
				...
				if (mMtgInterstitalVideoHandler != null) {
					/**
					 * @see com.mintegral.sdk.demo.InterstitialVideoActivity#onClick(View)
					 */
					mMtgInterstitalVideoHandler.setIVRewardEnable(MIntegralConstans.IVREWARD_TYPE_CLOSEMODE, 10);
					mMtgInterstitalVideoHandler.loadFromBid(bidToken);
				}
				break;
			case R.id.bt_show:
				if (mMtgInterstitalVideoHandler != null && mMtgInterstitalVideoHandler.isBidReady()) {
					mMtgInterstitalVideoHandler.showFromBid();
				}
				break;
			case R.id.bt_mute:
				if (mMtgInterstitalVideoHandler != null) {
	...
	public void showDialog(String RewardName, float RewardAmout) {
		try {
			mDialog = new Dialog(BidInterstitialVideoActivity.this);
			View view = View.inflate(BidInterstitialVideoActivity.this, R.layout.dialog_reward, null);
